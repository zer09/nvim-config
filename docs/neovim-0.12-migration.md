...
| `vim.lsp` | Unknown filetypes such as `typescript.tsx`, `javascript.jsx`, `gotmpl`, `yaml.docker-compose` | Mostly generated by upstream LSP configs. Leave documented as healthcheck noise. Add explicit filetype registrations only when a project actually uses one of those filetypes. |
